...
const exchangeColors: Record<string, string> = {
  ...
  deriv: 'text-green-400',
  deriv_live: 'text-green-500',
};
  ...
  async function handleAddKey(e: React.FormEvent) {
    ...
    try {
      ...
      const created = await api.createApiKey({
        ...
        is_testnet: newKey.exchange === 'bybit_testnet' || newKey.exchange === 'deriv',
      });
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-lg bg-accent border border-border flex items-center justify-center text-lg">
                      {key.exchange.startsWith('bybit') ? '⬡' : key.exchange.startsWith('deriv') ? '◉' : '◈'}
                    </div>
                    <div>
          ...
          <Dialog open={showAddKey} onOpenChange={setShowAddKey}>
            <DialogContent className="bg-card border-border max-w-md">
              ...
              <form onSubmit={handleAddKey} className="space-y-4 mt-2">
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent className="bg-card border-border">
                      ...
                      <SelectItem value="deriv">Deriv Forex (demo)</SelectItem>
                      <SelectItem value="deriv_live">Deriv Forex (live)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                ...
                {(newKey.exchange === 'deriv' || newKey.exchange === 'deriv_live') && (
                  <div className="p-3 bg-green-500/5 border border-green-500/15 rounded-lg space-y-2">
                    <p className="text-[10px] text-green-400 font-medium">
                      Deriv Forex {newKey.exchange === 'deriv' ? '(Demo)' : '(Live)'}
                    </p>
                    <p className="text-[10px] text-muted-foreground leading-relaxed">
                      <strong>Step 1</strong>: Create a free account at <strong>app.deriv.com</strong> {newKey.exchange === 'deriv' ? '(choose Virtual Money for demo)' : ''}
                      <br />
                      <strong>Step 2</strong>: Go to Settings → API Tokens → create token with <strong>Trade</strong> + <strong>Read</strong> scope
                      <br />
                      <strong>API Key</strong>: Your Deriv API token
                      <br />
                      <strong>API Secret</strong>: Enter <strong>1089</strong> (default app ID) or your custom Deriv app ID
                      <br />
                      Use forex pairs like EURUSD, GBPJPY, XAUUSD when creating agents.
                    </p>
                    {newKey.exchange === 'deriv_live' && (
                      <p className="text-[10px] text-red-400 font-medium mt-1">
                        Live trading uses real money. Start with demo mode first.
                      </p>
                    )}
                  </div>
                )}
