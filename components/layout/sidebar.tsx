...
export function Sidebar({ mobileOpen, onMobileClose }: SidebarProps) {
  ...
  useEffect(() => {
    onMobileClose();
    // We intentionally exclude onMobileClose from deps — the layout passes a
    // fresh closure each render, which would re-fire this effect every time.
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'flex flex-col bg-card border-r border-border transition-[width,transform] duration-300',
      ...
      >
        ...
        <button
          ...
          className="hidden md:flex absolute -right-3 top-20 w-6 h-6 rounded-full bg-card border border-border items-center justify-center hover:bg-accent transition-colors z-10"
        ...
        <div
          className={cn(
            'mx-4 mt-4 mb-2 flex items-center gap-2 px-3 py-2 bg-green-500/5 border border-green-500/15 rounded-lg',
            collapsed && 'md:hidden',
          )}
        >
          ...
          {navItems.map(({ href, icon: Icon, label }) => {
            ...
            return (
              ...
              >
                ...
                {active && (
                  <div className={cn('ml-auto w-1.5 h-1.5 rounded-full bg-primary', collapsed && 'md:hidden')} />
                )}
              </Link>
            );
          })}
