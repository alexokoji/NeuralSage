...
async def run_trading_tick_for_all_agents() -> dict:
    ...
    async with SessionLocal() as db:
        ...
        agent_ids = [row for row in result.scalars().all()]

    # Process each agent in its own session so a single failure doesn't roll
    # back the others.
...
async def run_optimization_sweep() -> dict:
    ...
    for agent in agents:
        try:
            async with SessionLocal() as db:
                # Refetch under this session — `agent` from the outer session is detached.
                a = await db.get(Agent, agent.id)
                if a is None or not a.strategy or not a.api_key_id or not a.trading_pairs:
                    continue
                api_key = await db.get(ApiKey, a.api_key_id)
                if api_key is None:
                    continue
                ...
                # Cross-agent warm start — top observations for the same strategy.
                ...
                # And contribute the observation back to the fleet KB.
                window_start = (
